...
export function Apply(socket: Socket, args: Uint8Array, callbackHandler: number, token: string) {
    ...
    sendResponsePacket(socket, "strategy.Apply", EMPTY_UINT8ARRAY, callbackHandler, token)
    sendTacticInfo(socket, callbackHandler, token)
}

export function sendTacticInfo(socket: Socket, callbackHandler: number, token: string) {
    const player = socketPlayerMap.get(socket)!
